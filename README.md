...
taux는 Claude Code 세션을 **하나의 tmux 대시보드**에서 모니터링하고 관리하는 TUI 도구입니다. 에이전트를 오케스트레이션하는 도구가 아닙니다. 이미 돌고 있는 세션들을 **잘 보여주는 것**에 집중합니다. htop이 프로세스를 만들지 않듯이, taux는 에이전트를 만들지 않습니다.
...
taux is a tmux-native TUI dashboard that monitors and manages Claude Code sessions from a single pane. It's not an agent orchestrator — it focuses on **showing you what's already running**. Just like htop doesn't create processes, taux doesn't create agents.
