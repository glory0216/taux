...
- **kubectl-style CLI**: `taux get sessions`, `taux describe`, `taux logs`, `taux attach`, `taux kill`, `taux delete`, `taux memorize`, `taux clean`, `taux setup`, `taux uninstall`
- **tmux native integration**: `prefix + H/A/S` popups, status bar widget (`taux status`)
...
- **Session memorize**: archive conversations to markdown and optionally delete originals
- **Session alias**: press `n` to set a nickname, reflected in search
